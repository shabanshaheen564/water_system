...
class ComplaintController extends Controller
{
    ...
    public function update(UpdateComplaintRequest $request, Complaint $complaint): JsonResponse
    {
        ...
        if (array_key_exists('status', $validated)) {
            ...
            if ($validated['status'] !== $oldStatus) {
                ...
                if (! in_array($validated['status'], $validTransitions[$oldStatus] ?? [], true)) {
                    abort(422, "Invalid status transition from {$oldStatus} to {$validated['status']}.");
                }
            }
        }
        ...
        foreach ($updateFields as $field) {
            if (array_key_exists($field, $validated)) abort_unless($request->user()->can('complaints.update'), 403);
        }
        if (array_key_exists('assigned_to', $validated) && $validated['assigned_to'] !== null) {
            $this->validateUserActive($validated['assigned_to']);
        }
    ...
    public function destroy(Complaint $complaint): JsonResponse
    {
        if ($complaint->workOrders()->exists()) {
            return response()->json(['message' => 'Cannot delete a complaint that is linked to a work order.'], 422);
        }
    ...
    public function convertToWorkOrder(Request $request, Complaint $complaint): JsonResponse
    {
        ...
        if ($complaint->workOrders()->exists()) {
            return response()->json(['message' => 'This complaint is already linked to a work order.'], 422);
        }
        ...
        return DB::transaction(function () use ($validated, $complaint, $request) {
            ...
            return response()->json([
                'message' => 'Complaint converted to work order successfully.',
                'work_order' => [
                    'id' => $workOrder->id,
                    ...
                    'complaints' => $workOrder->complaints->map(fn ($item) => ['id' => $item->id, 'complaint_number' => $item->complaint_number, 'title' => $item->title, 'status' => $item->status])->values(),
                ],
            ], 201);
        });
    }
    ...
    public function addToExistingWorkOrder(Request $request, Complaint $complaint): JsonResponse
    {
        ...
        return DB::transaction(function () use ($validated, $complaint, $request) {
            ...
            $workOrder->load(['complaints:id,complaint_number,title,status', 'assignedTo:id,name,email', 'createdBy:id,name,email']);
            return response()->json([
                'message' => 'Complaint added to work order successfully.',
    ...
    private function formatComplaint(Complaint $complaint, bool $withWorkOrders = false): array
    {
        $data = [
            ...
            'reported_by' => $complaint->reportedBy ? ['id' => $complaint->reportedBy->id, 'name' => $complaint->reportedBy->name, 'email' => $complaint->reportedBy->email] : null,
            'assigned_to' => $complaint->assignedTo ? ['id' => $complaint->assignedTo->id, 'name' => $complaint->assignedTo->name, 'email' => $complaint->assignedTo->email] : null,
            'processed_by' => $complaint->processedBy ? ['id' => $complaint->processedBy->id, 'name' => $complaint->processedBy->name, 'email' => $complaint->processedBy->email] : null,
            'processed_at' => $complaint->processed_at?->toISOString(),
        ...
        if ($withWorkOrders && $complaint->relationLoaded('workOrders')) {
            $data['work_orders'] = $complaint->workOrders->map(fn ($wo) => [
                'id' => $wo->id,
