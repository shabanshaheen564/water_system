...
class UserWebController extends Controller
{
    public function index(): View
    {
        ...
        return view('users.index', ['users' => $users, 'title' => __('Users')]);
    }
    ...
    public function show(User $user): View
    {
        ...
        return view('users.show', ['user' => $user, 'title' => __('User Details')]);
    }
    ...
    public function update(UpdateUserRequest $request, User $user): RedirectResponse
    {
        ...
        if ($wasSystemOwner && ! in_array('System Owner', $roles, true)) {
            ...
            if ($activeSystemOwners === 0) return back()->withErrors(['roles' => __('Cannot remove the last active System Owner role.')])->withInput();
        }
    ...
    {
        if ($user->hasRole('System Owner')) return back()->withErrors(['delete' => __('System Owner users cannot be deleted.')]);
        if ($user->id === auth()->id()) return back()->withErrors(['delete' => __('You cannot delete your own account.')]);
        $user->delete();
    ...
    private function assignableRoles()
    {
        ...
        if (auth()->user()->hasRole('Admin') && ! auth()->user()->hasRole('System Owner')) $roles->where('name', '!=', 'System Owner');
        return $roles->get();
    }

    private function canAssignRoles(array $roles): bool
    {
        if (auth()->user()->hasRole('System Owner')) return true;
        if (auth()->user()->hasRole('Admin')) return ! in_array('System Owner', $roles, true);
        return false;
    }
}
