...
class WorkOrderWebTest extends TestCase
{
    ...
    public function test_user_can_create_work_order_directly_and_assign_it(): void
    {
        ...
        $this->actingAs($this->user)
            ->post('/work-orders', [
                'title' => 'فحص مضخة', 'description' => 'فحص وتشغيل المضخة', 'priority' => 'high',
                'assigned_to' => $this->user->id, 'notes' => 'مهمة مباشرة.',
            ])
            ->assertSessionHasNoErrors()
            ->assertRedirect();

        $this->assertDatabaseHas('work_orders', [
            'title' => 'فحص مضخة', 'status' => 'assigned', 'priority' => 'high',
            'assigned_to' => $this->user->id, 'created_by' => $this->user->id,
        ]);
    }

    public function test_user_can_view_work_order_list_and_details_with_linked_complaints(): void
    {
        $complaint = Complaint::create(['complaint_number' => 'CMP-910001', 'title' => 'تسرب في الخط', 'description' => 'تسرب مياه', 'status' => 'in_progress', 'priority' => 'high', 'reported_by' => $this->user->id]);
        $workOrder = WorkOrder::create(['work_order_number' => 'WO-910001', 'complaint_id' => $complaint->id, 'title' => 'إصلاح التسرب', 'description' => 'إصلاح الخط', 'status' => 'assigned', 'priority' => 'high', 'assigned_to' => $this->user->id, 'created_by' => $this->user->id]);

        $this->actingAs($this->user)->get('/work-orders')->assertOk()->assertSee('المهام')->assertSee('WO-910001')->assertSee('إصلاح التسرب');
        $this->actingAs($this->user)->get("/work-orders/{$workOrder->id}")->assertOk()->assertSee('الشكاوى المرتبطة')->assertSee('CMP-910001')->assertSee('تسرب في الخط');
    }

    public function test_completing_work_order_from_web_closes_all_linked_complaints(): void
    {
        $complaints = collect([1, 2])->map(fn (int $number) => Complaint::create(['complaint_number' => 'CMP-91000' . $number, 'title' => 'شكوى مشتركة ' . $number, 'description' => 'المشكلة نفسها', 'status' => 'in_progress', 'priority' => 'medium', 'reported_by' => $this->user->id]));
    ...
    public function test_inactive_user_cannot_be_assigned_from_work_order_page(): void
    {
        ...
        $workOrder = WorkOrder::create(['work_order_number' => 'WO-910003', 'title' => 'مهمة اختبار', 'description' => 'اختبار', 'status' => 'assigned', 'priority' => 'medium', 'assigned_to' => $this->user->id, 'created_by' => $this->user->id]);
        $this->actingAs($this->user)->put("/work-orders/{$workOrder->id}", ['status' => 'assigned', 'assigned_to' => $inactive->id, 'priority' => 'medium'])->assertStatus(422);
    }
}
