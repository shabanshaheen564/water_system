...
class OperationalMapTest extends TestCase
{
    ...
    public function test_complaint_viewer_receives_only_authorized_operational_data(): void
    {
        ...
        $response->assertJsonPath('permissions.complaints', true);
        $response->assertJsonPath('permissions.tasks', false);
        $response->assertJsonPath('permissions.datasets', false);
        $response->assertJsonPath('complaints.0.number', 'CMP-920001');
        $response->assertJsonCount(0, 'work_orders');
        $response->assertJsonCount(0, 'datasets');
        $this->assertNotNull($workOrder->id);
    }

    public function test_task_viewer_receives_tasks_located_from_linked_complaints(): void
    {
        $user = User::factory()->create();
        $user->givePermissionTo(Permission::findByName('tasks.view', 'web'));

        $complaint = Complaint::create([
            'complaint_number' => 'CMP-920002',
            'title' => 'كسر خط',
            'description' => 'بلاغ تجريبي',
            'status' => 'in_progress',
            'priority' => 'urgent',
    ...
    public function test_dataset_viewer_receives_spatial_dataset_metadata(): void
    {
        ...
        Dataset::create([
            'name' => 'water_assets_test',
            'display_name' => 'أصول المياه التجريبية',
            'description' => 'طبقة اختبار',
            'dataset_type' => 'asset',
            'is_active' => true,
            'is_spatial' => true,
            'geometry_type' => 'Point',
