...
@section('content')
    <div class="p-4 sm:p-6 lg:p-8">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900">{{ __('Create Dataset') }}</h1>
            <p class="text-gray-600 mt-1">{{ __('Create a new dataset for managing spatial or tabular data') }}</p>
        </div>

        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <form method="POST" action="{{ route('datasets.store') }}" class="p-6 space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" required
                            ...
                            placeholder="e.g. wells, parcels, municipalities">
                        @error('name')
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                        placeholder="{{ __('Optional description of the dataset') }}">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="dataset_type" id="dataset_type" required
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            <option value="official_layer" {{ old('dataset_type') === 'official_layer' ? 'selected' : '' }}>Official Layer</option>
                            <option value="additional_table" {{ old('dataset_type') === 'additional_table' ? 'selected' : '' }}>Additional Table</option>
                        </select>
                        @error('dataset_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="source_name" id="source_name" value="{{ old('source_name') }}"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                            placeholder="{{ __('e.g. Municipality GIS, Survey Dept') }}">
                        @error('source_name')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="text" name="source_format" id="source_format" value="{{ old('source_format') }}"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                            placeholder="{{ __('e.g. Shapefile, GeoJSON, CSV') }}">
                        @error('source_format')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex items-center">
                            ...
                            <input type="checkbox" name="is_spatial" id="is_spatial" value="1" {{ old('is_spatial') ? 'checked' : '' }}
                ...
                <div id="spatial-fields" class="hidden grid grid-cols-1 md:grid-cols-2 gap-6" style="display: {{ old('is_spatial') ? 'grid' : 'none' }};">
                    <div>
                    ...
                    <div>
                        ...
                        <input type="number" name="srid" id="srid" value="{{ old('srid') }}" min="1" max="999999"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
