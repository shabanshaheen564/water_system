        ...
        <div class="mb-6 flex items-center justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-ink-secondary">
                    {{ __('Browse and manage system users') }}
                </p>
            </div>
            ...
            @can('users.create')
                <a
                    ...
                    class="btn-motion shrink-0 rounded-md bg-brand-600 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700"
                >
                    {{ __('Create User') }}
                </a>
            @endcan
                    ...
                    <tbody>
                        @forelse($users as $user)
                            <tr class="hover:bg-surface-1">
                                ...
                                <td class="whitespace-nowrap">
                                    <div class="ltr-value text-sm text-ink-secondary">
                                        {{ $user->email }}
                                    </div>
                                </td>
                                ...
                                <td class="whitespace-nowrap">
                                    <span
                                        class="inline-flex rounded-md border px-2 py-1 text-xs font-medium {{ $user->is_active ? 'border-success bg-success-surface text-success' : 'border-border bg-surface-1 text-ink-secondary' }}"
                                    >
                                        ...
                                        @can('users.update')
                                            @if(!$user->hasRole('System Owner') || auth()->user()->hasRole('System Owner'))
                                                <a
                                                    ...
                                                    class="text-ink-secondary hover:text-ink"
                                                >
                                                    {{ __('Edit') }}
                                                </a>
                                            @endif
