...
@section('content')
@php($formatDuration = fn ($minutes) => \App\Support\DurationFormatter::format($minutes) ?? '—')
<div class="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
    ...
    <div class="mb-6 grid gap-4 md:grid-cols-3 lg:grid-cols-5">
        @foreach([
            ...
            ['label'=>'متوسط الاستجابة','value'=>$stats['avg_response_minutes'] !== null ? $formatDuration((int) round($stats['avg_response_minutes'])) : '—'],
            ['label'=>'متوسط زمن الحل','value'=>$stats['avg_resolution_minutes'] !== null ? $formatDuration((int) round($stats['avg_resolution_minutes'])) : '—'],
            ['label'=>'متوسط تنفيذ المهمة','value'=>$stats['avg_task_execution_minutes'] !== null ? $formatDuration((int) round($stats['avg_task_execution_minutes'])) : '—'],
        ] as $card)
    ...
    <div class="card-institutional mb-6 overflow-hidden">
        ...
        @forelse($complaints as $item)
            <tr><td class="ltr-value">{{ $item->complaint_number }}</td><td>{{ $item->title }}</td><td>{{ $item->priority }}</td><td>{{ $formatDuration($item->response_time_minutes) }}</td><td>{{ $formatDuration($item->resolution_time_minutes) }}</td><td class="ltr-value">{{ $item->archived_at?->format('Y-m-d H:i') }}</td></tr>
    ...
    <div class="card-institutional overflow-hidden">
        ...
        @forelse($workOrders as $item)
            <tr><td class="ltr-value">{{ $item->work_order_number }}</td><td>{{ $item->title }}</td><td>{{ $item->priority }}</td><td>{{ $formatDuration($item->response_time_minutes) }}</td><td>{{ $formatDuration($item->execution_time_minutes) }}</td><td class="ltr-value">{{ $item->archived_at?->format('Y-m-d H:i') }}</td></tr>
        @empty <tr><td colspan="6" class="py-10 text-center text-sm text-ink-muted">لا توجد سجلات مؤرشفة.</td></tr>@endforelse
        </tbody></table></div>
        {{ $workOrders->links() }}
    </div>
