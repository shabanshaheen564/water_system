...
<div class="mx-auto max-w-6xl p-4 sm:p-6 lg:p-8">
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            @endcan
            @can('complaints.update')<a href="{{ route('complaints.edit', $complaint) }}" class="rounded-md border border-border-strong bg-white px-4 py-2 text-sm font-medium text-ink hover:bg-surface-1">معالجة / إسناد</a>@endcan
            ...
            <section data-enter class="card-institutional overflow-hidden lg:col-span-3">
                ...
                <div class="overflow-x-auto">
                    <table class="table-institutional">
                        <thead><tr><th>رقم المهمة</th><th>العنوان</th><th>المسؤول</th><th>الحالة</th><th>الأولوية</th><th>عدد الشكاوى</th></tr></thead>
                        <tbody>
                            @foreach($complaint->workOrders as $workOrder)
                                <tr>
                                    <td><code class="ltr-value text-sm">{{ $workOrder->work_order_number }}</code></td>
                                    <td>{{ $workOrder->title }}</td>
                                    <td>{{ $workOrder->assignedTo->name ?? '—' }}</td>
                                    <td>{{ ['pending'=>'معلقة','assigned'=>'مسندة','in_progress'=>'قيد التنفيذ','completed'=>'مكتملة','cancelled'=>'ملغاة'][$workOrder->status] ?? $workOrder->status }}</td>
                                    <td>{{ ['low'=>'منخفضة','medium'=>'متوسطة','high'=>'عالية','urgent'=>'عاجلة'][$workOrder->priority] ?? $workOrder->priority }}</td>
                                    <td>{{ $workOrder->complaints->count() }}</td>
                                </tr>
                                @if($workOrder->complaints->count() > 1)
                                    <tr><td colspan="6" class="bg-surface-1"><div class="text-xs text-ink-muted">الشكاوى المرتبطة بهذه المهمة:</div><div class="mt-2 flex flex-wrap gap-2">@foreach($workOrder->complaints as $linkedComplaint)<span class="rounded border border-border bg-white px-2 py-1 text-xs text-ink"><code class="ltr-value text-brand-600">{{ $linkedComplaint->complaint_number }}</code> — {{ $linkedComplaint->title }}</span>@endforeach</div></td></tr>
                                @endif
                            @endforeach
                        </tbody>
                    </table>
