    ...
    <form method="GET" class="card-institutional mb-5 grid gap-4 p-4 md:grid-cols-4">
        ...
        <div>
            ...
            <select id="status" name="status" class="input-institutional w-full text-sm">
                ...
                <option value="open" @selected($status === 'open')>جديدة</option>
                <option value="in_progress" @selected($status === 'in_progress')>قيد المعالجة</option>
                <option value="resolved" @selected($status === 'resolved')>تم الحل</option>
        ...
        <div class="md:col-span-4 flex gap-2 border-t border-border pt-4">
            ...
            <a href="{{ route('complaints.index') }}" class="rounded-md border border-border-strong bg-white px-4 py-2 text-sm font-medium text-ink hover:bg-surface-1">مسح</a>
        </div>
    </form>
    ...
    <div data-enter class="card-institutional overflow-hidden">
        <div class="overflow-x-auto">
            <table class="table-institutional">
                <thead><tr>
                    <th>رقم الشكوى</th><th>العنوان</th><th>المواطن</th><th>الأولوية</th><th>الحالة</th><th>المسند إليه</th><th>التاريخ</th><th>الإجراءات</th>
                </tr></thead>
                <tbody>
                @forelse($complaints as $complaint)
                    <tr class="hover:bg-surface-1">
                        ...
                        <td class="whitespace-nowrap">
                            ...
                            <span class="inline-flex rounded-md border px-2 py-1 text-xs font-medium {{ $priorityClasses[$complaint->priority] ?? $priorityClasses['medium'] }}">{{ $priorityLabels[$complaint->priority] ?? $complaint->priority }}</span>
                        </td>
                        <td class="whitespace-nowrap">
                            @php($statusLabels = ['open'=>'جديدة','in_progress'=>'قيد المعالجة','resolved'=>'تم الحل','closed'=>'مغلقة','cancelled'=>'ملغاة'])
                            <span class="inline-flex rounded-md border border-border bg-surface-1 px-2 py-1 text-xs font-medium text-ink-secondary">{{ $statusLabels[$complaint->status] ?? $complaint->status }}</span>
                        </td>
                        ...
                        <td class="whitespace-nowrap">
                            <div class="flex items-center gap-3 text-sm font-medium">
                                <a href="{{ route('complaints.show', $complaint) }}" class="text-brand-600 hover:text-brand-700">عرض</a>
                                @can('complaints.update')<a href="{{ route('complaints.edit', $complaint) }}" class="text-ink-secondary hover:text-ink">تعديل</a>@endcan
                                @can('complaints.delete')
                                    @if(!$complaint->workOrders->count())
                                        <form method="POST" action="{{ route('complaints.destroy', $complaint) }}" class="inline" onsubmit="return confirm('هل أنت متأكد من حذف الشكوى؟');">@csrf @method('DELETE')<button class="text-danger hover:underline">حذف</button></form>
                                    @endif
                                @endcan
                            </div>
                        </td>
